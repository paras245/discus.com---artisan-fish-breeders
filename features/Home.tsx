...
const Home: React.FC = () => {
  ...
  const { lang } = useApp();
  const [selectedFish, setSelectedFish] = useState<typeof FISH_PRODUCTS[0] | null>(null);
    ...
    <main className="pb-20">
      ...
      <section className="relative h-[450px] md:h-[650px] overflow-hidden">
        ...
        <div className="relative h-full max-w-7xl mx-auto px-4 flex flex-col justify-center text-sky-950">
          ...
          <h1 className="text-5xl md:text-8xl font-black mb-6 leading-[0.9] tracking-tighter animate-fade-in-up [animation-delay:0.1s]">
            The King of <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-900 to-indigo-800">Aquariums</span>
          </h1>
          <p className="text-lg md:text-2xl mb-10 max-w-xl font-medium text-sky-800/80 leading-relaxed animate-fade-in-up [animation-delay:0.2s]">
            Artisan discus breeders and importers since 1996. Delivering the world's most vibrant tropical specimens to your doorstep.
          </p>
      ...
      <section className="max-w-7xl mx-auto px-4 py-24">
        <div className="flex flex-col items-center mb-16">
          <span className="text-yellow-600 font-black text-xs uppercase tracking-[0.3em] mb-4">Explore our world</span>
          <h2 className="text-4xl font-black text-sky-950 mb-4 tracking-tighter">Shop By Category</h2>
          ...
          {CATEGORIES.map(cat => (
            <div key={cat.id} className="group cursor-pointer">
              <div className="relative aspect-square rounded-[2rem] overflow-hidden mb-4 shadow-sm border-2 border-white group-hover:shadow-2xl transition-all group-hover:border-sky-200">
                <img src={cat.image} alt={cat.name[lang]} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
              ...
              <h3 className="text-center font-black text-sm text-sky-950 group-hover:text-sky-600 transition-colors tracking-tight">{cat.name[lang]}</h3>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-16">
            <div className="max-w-xl">
              <span className="text-yellow-600 font-black text-xs uppercase tracking-[0.3em] mb-4 block">Hand-Selected</span>
              <h2 className="text-4xl md:text-5xl font-black text-sky-950 mb-4 tracking-tighter">Artisan Showcase</h2>
              <p className="text-sky-700 font-medium opacity-70">Exotic specimens with high-contrast colors and perfect symmetry, ready for competitive display.</p>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-24">
        <div className="relative bg-sky-950 rounded-[3rem] overflow-hidden p-12 md:p-24 flex flex-col items-center text-center text-white shadow-3xl">
           ...
           <span className="text-yellow-500 font-black text-xs uppercase tracking-[0.4em] mb-6 relative z-10">Exclusive Access</span>
           <h2 className="text-4xl md:text-6xl font-black mb-8 relative z-10 tracking-tighter max-w-3xl leading-none">Join the King of Aquarium Club</h2>
           <p className="text-sky-200/80 max-w-xl mb-12 text-lg font-medium relative z-10 leading-relaxed">Early access to new imports, rare discus auctions, and professional breeding guides delivered weekly.</p>
           ...
           <div className="flex flex-col sm:flex-row gap-4 w-full max-w-lg relative z-10">
             ...
             />
             <button className="bg-yellow-500 hover:bg-yellow-400 text-black px-12 py-5 rounded-2xl font-black shadow-xl transition-all transform hover:-translate-y-1">JOIN NOW</button>
           </div>
        </div>
      </section>
